...
const DEFAULT_SECTIONS = [
  { key:'emotional', icon:'💛', label:'Emotional Quotient', color:'#AD1457', desc:'How emotionally aware and mature is he?', questions:[
  ...
  { key:'financial', icon:'💰', label:'Financial & Career', color:'#1D9E75', desc:'Stability, ambition, and financial responsibility', questions:[
...
function gsty(s) { return s>=82?{bg:'#E8F5E9',col:'#2E7D32'}:s>=70?{bg:'#FFF8E1',col:'#F57F17'}:{bg:'#FFEBEE',col:'#C62828'} }
function isUnrated(p, sections) { return sections.flatMap(s=>s.questions).every(q=>!(p.params||{})[q.k]) }
function defaultParams(sections) { return Object.fromEntries(sections.flatMap(s=>s.questions).map(q=>[q.k,0])) }
...
function HeartSlider({ value, onChange, color }) {
  ...
  return (
    <input type="range" min="0" max="100" step="1" value={value} onChange={onChange}
      style={{WebkitAppearance:'none',appearance:'none',width:'100%',height:'4px',borderRadius:'2px',outline:'none',cursor:'pointer',
        background:`linear-gradient(to right,${color||'#C2185B'} ${pct}%,#f5e6ec ${pct}%)`}} />
  )
}
...
export default function Dashboard({ session }) {
  ...
  function editQLabel(si, qi) {
    ...
    if (newLabel && newLabel.trim()) {
      updateSections(sections.map((s,i) => i===si ? {...s, questions: s.questions.map((q,j)=>j===qi?{...q,label:newLabel.trim()}:q)} : s))
    }
  }
  ...
  function addQ(si) {
    ...
    updateSections(sections.map((s,i) => i===si ? {...s, questions:[...s.questions,{k,label:label.trim(),hint:hint.trim()}]} : s))
  ...
  async function saveProspect() {
    ...
    const payload = {
      ...
      name:form.name, age:parseInt(form.age)||28, city:form.city||'', hometown:form.hometown||'',
      job:form.job||'', company:form.company||'', edu:form.edu||'', height:form.height||'',
      zodiac:form.zodiac||'Aries', emoji:form.emoji||'👤', color:form.color||'#FCE4EC',
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#FFF0F5',fontFamily:'DM Sans,sans-serif'}}>
      <div style={{textAlign:'center'}}><div style={{fontSize:'32px',marginBottom:'1rem'}}>💍</div><div style={{color:'#C2185B',fontSize:'14px'}}>Loading your prospects...</div></div>
    </div>
  )
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',fontFamily:'DM Sans,sans-serif',background:'#FFFAF8'}}>
      ...
      <aside style={{width:'240px',minWidth:'240px',background:'linear-gradient(160deg,#fff0f5,#fff8f0)',borderRight:'1px solid rgba(194,24,91,0.13)',display:'flex',flexDirection:'column',position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
        <div style={{padding:'1.25rem 1rem .75rem',borderBottom:'1px solid rgba(194,24,91,0.13)'}}>
          <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
            <div style={{width:'36px',height:'36px',borderRadius:'10px',background:'linear-gradient(135deg,#C2185B,#F48FB1)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,boxShadow:'0 2px 8px rgba(194,24,91,0.3)'}}>
              <svg viewBox="0 0 24 24" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="white"/>
              </svg>
            </div>
            <div>
              <div style={{fontFamily:'Playfair Display,serif',fontSize:'17px',color:'#C2185B',lineHeight:1}}>Rishta Radar</div>
              <div style={{fontSize:'10px',color:'#B39DAE',marginTop:'2px'}}>Your match, your way</div>
            </div>
          </div>
          <div style={{fontSize:'10px',color:'#7B5E6B',marginTop:'8px'}}>{session.user.email}</div>
        </div>
        ...
        <div style={{flex:1,padding:'.4rem 7px .75rem',overflowY:'auto'}}>
          {favourites.length ? favourites.map(p=>{
            const s=gsc(p,sections),st=gsty(s),unr=isUnrated(p,sections)
            return (
              <div key={p.id} onClick={()=>setFavModal(p)}
                style={{display:'flex',alignItems:'center',gap:'7px',padding:'7px 9px',borderRadius:'9px',cursor:'pointer',marginBottom:'2px',background:'transparent'}}>
          ...
          }) : <div style={{fontSize:'11px',color:'#B39DAE',padding:'8px 10px',fontStyle:'italic'}}>No favourites yet</div>}
        </div>
      ...
      <main style={{flex:1,padding:'1.75rem',overflowY:'auto',minWidth:0}}>
        ...
        {page==='overview' && (
          <div>
            ...
            {ovTab==='kanban' && <KanbanBoard prospects={prospects} sections={sections} onMove={updateStatus} onSelect={selAndView} dragId={dragId} setDragId={setDragId} />}
            {ovTab==='detail' && <ProfileView prospects={prospects} sections={sections} selId={selId} setSelId={setSelId} onGoToProspects={(id)=>{setPage('prospects');setSelId(id)}} />}
          </div>
        )}
        ...
        {page==='prospects' && (
          <div>
            ...
            {prospects.length===0 && <div style={{color:'#7B5E6B',fontSize:'13px',padding:'1rem'}}>No prospects yet — add one!</div>}
            {prospects.map(p=>(
              <div key={p.id} id={`ap-${p.id}`} style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.1rem',marginBottom:'14px'}}>
                ...
                <div style={{marginTop:'12px'}}>
                  <div style={{fontSize:'10px',fontWeight:'500',textTransform:'uppercase',letterSpacing:'.8px',color:'#B39DAE',marginBottom:'6px'}}>📝 My Notes</div>
      ...
      {/* FAV SUMMARY MODAL */}
      {favModal && (
        <div onClick={e=>{if(e.target===e.currentTarget)setFavModal(null)}}
          style={{position:'fixed',inset:0,background:'rgba(44,24,16,.4)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:'16px',padding:'1.75rem',width:'420px',maxWidth:'95vw',maxHeight:'88vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(194,24,91,.2)'}}>
            <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'1.25rem'}}>
              <div style={{fontSize:'36px',background:favModal.color,borderRadius:'50%',width:'60px',height:'60px',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{favModal.emoji}</div>
              <div style={{flex:1}}>
                <div style={{fontFamily:'Playfair Display,serif',fontSize:'20px',color:'#2C1810'}}>{favModal.name}</div>
                <div style={{fontSize:'12px',color:'#7B5E6B',marginTop:'2px'}}>{favModal.age}y · {favModal.city} · {favModal.job}</div>
                {spill(favModal.status)}
              </div>
              <div style={{textAlign:'center',flexShrink:0}}>
                <div style={{fontFamily:'Playfair Display,serif',fontSize:'36px',fontWeight:'600',color:'#C2185B',lineHeight:1}}>{gsc(favModal,sections)}</div>
                <div style={{fontSize:'9px',color:'#B39DAE'}}>score</div>
              </div>
            </div>
            {sections.map(sec=>{
              const ss=secScore(favModal,sec)
              return (
                <div key={sec.key} style={{marginBottom:'8px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:'3px'}}>
                    <span style={{fontSize:'11px',color:'#7B5E6B'}}>{sec.icon} {sec.label}</span>
                    <span style={{fontSize:'11px',fontWeight:'600',color:sec.color}}>{ss}</span>
                  </div>
                  <div style={{height:'5px',background:'#f5e6ec',borderRadius:'3px'}}>
                    <div style={{height:'5px',background:sec.color,borderRadius:'3px',width:ss+'%'}} />
                  </div>
                </div>
              )
            })}
            {favModal.notes && (
              <div style={{marginTop:'1rem',padding:'.75rem',background:'#FFF0F5',borderRadius:'10px'}}>
                <div style={{fontSize:'10px',color:'#B39DAE',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'.5px'}}>My Notes</div>
                <div style={{fontSize:'13px',color:'#2C1810',lineHeight:1.6}}>{favModal.notes}</div>
              </div>
            )}
            <div style={{display:'flex',gap:'8px',marginTop:'1.25rem'}}>
              <button onClick={()=>{selAndView(favModal.id);setFavModal(null)}}
                style={{flex:1,padding:'8px',borderRadius:'18px',background:'#C2185B',color:'#fff',border:'none',cursor:'pointer',fontFamily:'DM Sans,sans-serif',fontSize:'12px',fontWeight:'500'}}>
                View full profile →
              </button>
              <button onClick={()=>setFavModal(null)}
                style={{padding:'8px 16px',borderRadius:'18px',background:'#fff',border:'1px solid rgba(194,24,91,0.13)',color:'#7B5E6B',cursor:'pointer',fontFamily:'DM Sans,sans-serif',fontSize:'12px'}}>
                Close
              </button>
            </div>
          </div>
...
function ProfileView({ prospects, sections, selId, setSelId, onGoToProspects }) {
  ...
  const unr = p ? isUnrated(p,sections) : false

  return (
    <div>
      {/* Prospect switcher */}
      <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'12px',padding:'.75rem 1rem',marginBottom:'1.25rem'}}>
        <div style={{fontSize:'10px',fontWeight:'500',color:'#B39DAE',textTransform:'uppercase',letterSpacing:'.5px',marginBottom:'8px'}}>Select a prospect</div>
      ...
      {!p ? (
        <div style={{textAlign:'center',padding:'3rem',color:'#7B5E6B'}}>
          <div style={{fontSize:'32px',marginBottom:'1rem'}}>👆</div>
          <div style={{fontSize:'14px'}}>Pick a prospect above to view their profile</div>
        </div>
      ) : (
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px'}}>
          {/* Hero */}
          <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.5rem',gridColumn:'1/-1',display:'flex',alignItems:'flex-start',gap:'1.25rem',flexWrap:'wrap'}}>
            <div style={{fontSize:'48px',background:p.color,borderRadius:'50%',width:'72px',height:'72px',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{p.emoji}</div>
            <div style={{flex:1,minWidth:'200px'}}>
              <div style={{fontFamily:'Playfair Display,serif',fontSize:'22px',color:'#2C1810'}}>{p.name}</div>
              <div style={{fontSize:'12px',color:'#7B5E6B',marginTop:'4px',lineHeight:1.7}}>
                {p.age}y · {p.city}{p.hometown?' · from '+p.hometown:''}<br/>
                {p.job}{p.company?' @ '+p.company:''}<br/>
                {[p.edu,p.height,p.zodiac].filter(Boolean).join(' · ')}
              </div>
              <div style={{marginTop:'8px'}}>{spill(p.status)}</div>
              <button onClick={()=>onGoToProspects(p.id)}
                style={{marginTop:'12px',padding:'7px 16px',borderRadius:'18px',background:'#C2185B',color:'#fff',border:'none',cursor:'pointer',fontFamily:'DM Sans,sans-serif',fontSize:'12px',fontWeight:'500'}}>
                ✏️ Edit / Rate this prospect →
              </button>
            </div>
            <div style={{textAlign:'center',flexShrink:0}}>
          ...
          <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.1rem'}}>
            ...
            {(p.greens||[]).length>0 && <div style={{marginBottom:'8px'}}><div style={{fontSize:'10px',color:'#B39DAE',marginBottom:'5px'}}>GREEN FLAGS</div><div style={{display:'flex',flexWrap:'wrap',gap:'5px'}}>{p.greens.map(g=><span key={g} style={{fontSize:'11px',padding:'3px 9px',borderRadius:'9px',background:'#E8F5E9',color:'#2E7D32'}}>{g}</span>)}</div></div>}
          ...
          {/* Family */}
          {[{label:'Family type',val:p.famtype},{label:'Family members',val:p.famsize},{label:'Parents',val:p.parents},{label:'Siblings',val:p.siblings},{label:'Income',val:p.income},{label:'Living',val:p.living},{label:'After marriage',val:p.cityplan}].filter(r=>r.val).length>0 && (
            <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.1rem',gridColumn:'1/-1'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:'8px'}}>
                {[{label:'Family type',val:p.famtype},{label:'Family members',val:p.famsize},{label:'Parents',val:p.parents},{label:'Siblings',val:p.siblings},{label:'Annual income',val:p.income},{label:'Living situation',val:p.living},{label:'After marriage',val:p.cityplan}].filter(r=>r.val).map(r=>(
                  <div key={r.label} style={{display:'flex',gap:'8px'}}>
                    <span style={{fontSize:'11px',color:'#B39DAE',flexShrink:0}}>{r.label}:</span>
    ...
    return (
      <div key={sec.key} style={{border:'1px solid rgba(194,24,91,0.13)',borderRadius:'10px',overflow:'hidden'}}>
        <div onClick={()=>toggle(si)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px 14px',cursor:'pointer',background:'#fff',userSelect:'none'}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:'13px',fontWeight:'500',color:'#2C1810'}}>{sec.label}</div><div style={{fontSize:'10px',color:'#7B5E6B',marginTop:'1px'}}>{sec.desc}</div></div>
          <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
            <div style={{width:'60px',height:'5px',background:'#f5e6ec',borderRadius:'3px',overflow:'hidden'}}><div style={{width:ss+'%',height:'5px',borderRadius:'3px',background:sec.color}} /></div>
            ...
            <span style={{fontSize:'11px',color:'#B39DAE',display:'inline-block',transform:isOpen?'rotate(0)':'rotate(-90deg)',transition:'transform .2s'}}>▼</span>
          </div>
        </div>
        {isOpen && (
...
// ── KANBAN BOARD ──────────────────────────────────────────────────────────────
function KanbanBoard({ prospects, sections, onMove, onSelect, dragId, setDragId }) {
  return (
          ...
          <div key={col.key} style={{flex:'0 0 210px',minWidth:'210px',background:col.colBg,border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',overflow:'hidden'}}
            ...
            <div style={{padding:'10px 10px 14px',display:'flex',flexDirection:'column',gap:'9px',minHeight:'80px'}}>
              {cards.length ? cards.map(p=>{
                const s=gsc(p,sections),st=gsty(s),unr=isUnrated(p,sections)
                return (
                  <div key={p.id} draggable onDragStart={()=>setDragId(p.id)} onDragEnd={()=>setDragId(null)} onClick={()=>onSelect(p.id)}
                    ...
                    <div style={{display:'flex',alignItems:'flex-start',gap:'8px',marginBottom:'7px'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:'10px',color:'#7B5E6B'}}>{p.job}</div>
                      </div>
                      <div style={{textAlign:'center',flexShrink:0}}>
                        {unr ? <div style={{fontSize:'9px',color:'#B39DAE',fontStyle:'italic',marginTop:'4px'}}>Not rated</div>
    ...
    <div>
        ...
        <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.1rem'}}>
          ...
          {milSorted.map(p=>{const v=milScore(p),c=v>=80?'#4CAF50':v>=65?'#FF9800':'#F44336',em=v>=80?'😍':v>=65?'🙂':'😬';return(
            <div key={p.id} style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'7px'}}>
              ...
              <div style={{flex:1,height:'7px',background:'#f5e6ec',borderRadius:'3px',overflow:'hidden'}}><div style={{width:v+'%',height:'7px',borderRadius:'3px',background:c}} /></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px'}}>
        ...
        <div style={{background:'#fff',border:'1px solid rgba(194,24,91,0.13)',borderRadius:'16px',padding:'1.1rem'}}>
          ...
          {active.map(p=>{const s=gsc(p,sections),st=gsty(s),unr=isUnrated(p,sections),cp=s>=85?{stars:'★★★★★',lbl:'Strong match!',c:'#2E7D32'}:s>=78?{stars:'★★★★☆',lbl:'Good potential',c:'#F57F17'}:s>=68?{stars:'★★★☆☆',lbl:'Worth exploring',c:'#F57F17'}:{stars:'★★☆☆☆',lbl:'Think carefully',c:'#C62828'};return(
            <div key={p.id} style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px',paddingBottom:'8px',borderBottom:'1px solid rgba(194,24,91,0.1)'}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:'12px',fontWeight:'500'}}>{p.name}</div><div style={{fontSize:'11px'}}>{cp.stars} <span style={{color:cp.c}}>{cp.lbl}</span></div></div>
